...
export default function EditService({ params }) {
    ...
    useEffect(() => {
        const getServiceData = async () => {
            try {
                ...
                const docRef = await doc(db, "raloc/travels/service", serviceId);
                const docSnap = await getDoc(docRef);

                const resultsData = docSnap.data()

                if (resultsData) {
                    setFormData((prevData) => ({ ...prevData, service: resultsData.service, description: resultsData.description, serviceImagePreview: resultsData.serviceImage, serviceImage: resultsData.serviceImage, id: resultsData.id }))
                } else {
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            else if (!formData.serviceImageFile && !formData.serviceImagePreview) {
                toast.error("Please select an image!")
                return
            }

            const data = {
                service: formData.service, description: formData.description, serviceImage: getImageUrl ? getImageUrl : formData.serviceImage, updatedAt: new Date().toISOString()
            }

            // Reference to the document
            const docRef = doc(db, `raloc/travels/service/${serviceId}`);
